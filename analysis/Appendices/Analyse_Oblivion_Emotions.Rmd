...
Parse emotion and intensity into seperate columns:
...
There is a significant association between the gender of the NPC and masculinity/femininty of the emotion (`r chisqEGString`). Male NPCs are more likely to use masculine emotions (anger) than female NPCs, and female NPCs are more likely to use feminine emotions (happiness, sadness, surprise, fear and disgust). 

Testing the difference between the "backgrounding" and gendered emotions hypotheses is hard. The "neutral" emotions aren't masculine or feminie, and the analysis above removes them from the data. But this is the key distinction for the backgrounding hypothesis. So the two variables can't be put together in a regression, and the two analyses above were based on different data, so can't be compared directly. 
...
It looks like the distribution for males is more biased towards low ranges of emotions, while the distribution for females is slighly higher.

However, this effect might be confounded by the amount of dialouge. There is no significant effect of gender on the number of emotions displayed when the (log) number of lines is considered:
